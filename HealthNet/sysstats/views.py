...
def stats(request):
    ...
    info_views = logs.filter(type__contains="Medical Info viewed/updated").filter(time__range=[startDate, endDate])

    loglabels = ["Logins", "Logouts", "Registrations", "Appointment creations", "Transfers", "Medical Info Views"]
    ...
    context = {'Users': zip(labels, counts), 'Logs': zip(loglabels, logcounts), 'Message_count': messages.count(),
               ...
               'Test_result_labels': tr_labels, 'Test_results': tr_list,
